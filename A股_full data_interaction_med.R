...
# 
# data_long_clean <- data_long_clean %>% mutate(across(c(asset_turnover, ins_hold, CR), fill_median))
# 
...
# 相关性矩阵
correlation_matrix <- cor(data_long_clean[, c("ESG", "ln_mc", "ln_size", "ROE", 
...
#国企/私企/外资
#basic
model_interaction_soe <- lm(ln_mc ~ ESG * nature_soe + ln_size + ROE + leverage + 
                              tobinq + CR + factor(year) + field_id, data = data_long_clean)
summary(model_interaction_soe)

# 固定效应国企/私企/外资
model_fe <- plm(ln_mc ~ ESG * nature_soe + ln_size + ROE + leverage + 
                  tobinq + CR + year, 
...
model_re <- plm(ln_mc ~ ESG * nature_soe + ln_size + ROE + leverage + 
                  tobinq + CR + factor(year) + field_id, 
...
model_interaction_pollute <- lm(ln_mc ~ ESG * field_id + ln_size + 
                                  ROE + leverage + tobinq + nature_soe + factor(year), 
                                data = data_long_clean)
summary(model_interaction_pollute)

# 固定效应污染/重污染
model_fe2 <- plm(ln_mc ~ ESG * field_id + ln_size + ROE + leverage + 
...
# 随机效应污染/重污染
model_re2 <- plm(ln_mc ~ ESG * field_id + ln_size + ROE + leverage + 
                   tobinq + CR + factor(year), 
                 data = data_long_clean, index = c("code", "year"), model = "random")
summary(model_re2)
...
# OLS 基础回归
model_basic <- lm(ln_mc ~ ESG + ln_size + ROE + leverage + 
                    tobinq + CR + nature_soe + field_id + factor(year), 
                  data = data_long_clean)
summary(model_basic) 

# 固定效应
model_fe3 <- plm(ln_mc ~ ESG + ln_size + ROE + leverage + 
                   tobinq + CR + nature_soe + field_id + factor(year), data = data_long_clean, 
                 index = c("code", "year"), model = "within")
summary(model_fe3)

# 随机效应
model_re3 <- plm(ln_mc ~ ESG + ln_size + ROE + leverage + 
                   tobinq + CR + nature_soe + field_id + factor(year), data = data_long_clean, 
...
# esg to 资产周转率
model_esg_at <- lm(formula = asset_turnover ~ ESG + ln_size + ROE + leverage + tobinq + 
                     CR + field_id + year + nature_soe, data = data_long_clean)


# esg to 机构持股
model_esg_ih <-lm(ins_hold ~ ESG + ln_size + ROE + leverage + tobinq + CR + 
                    field_id + year + nature_soe, data = data_long_clean)

# esg to IR
model_esg_ir <-lm(IR ~ ESG + ln_size + ROE + leverage + tobinq + CR + 
                    field_id + year + nature_soe, data = data_long_clean)

# 资产周转率 to mc
model_med1 <- lm( ln_mc ~ ESG + asset_turnover + ln_size + ROE + leverage + 
                    tobinq + CR + field_id + year + nature_soe, data = data_long_clean)


# 机构持股 to mc
model_med2 <- lm(ln_mc ~ ESG + ins_hold + ln_size + ROE + leverage + 
                   tobinq + CR + field_id + year + nature_soe, data = data_long_clean)

#IR to mc
model_med3 <- lm(ln_mc ~ ESG + IR + ln_size + ROE + leverage + 
                   tobinq + CR + field_id + year + nature_soe, data = data_long_clean)
